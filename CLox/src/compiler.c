...
static void super_(bool canAssign)
{
    ...
    if (match(TOKEN_LEFT_PAREN))
    {
        ...
        writeConstantOpcode(OP_SUPER_INVOKE, OP_SUPER_INVOKE_LONG, name, parser.previous.line);
        emitByte(argCount);
    }
    else
    {
        namedVariable(syntheticToken("super"), false);
        writeConstantOpcode(OP_GET_SUPER, OP_GET_SUPER_LONG, name, parser.previous.line);
    }
}
...
static void forStatement()
{
    ...
    if (!match(TOKEN_RIGHT_PAREN))
    {
        ...
        expression();
        emitByte(OP_POP);
        consume(TOKEN_RIGHT_PAREN, "Expect ')' for clauses.");
...
static void whileStatement()
{
    ...
    expression();
    consume(TOKEN_RIGHT_PAREN, "Expect ')' aftr condition.");
